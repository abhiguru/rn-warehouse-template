...
merged a research recipe for rebuilding Grafana's server binary with patched Thrift.
The native amd64 candidate passed startup, plugin-signature and live-query
checks. Its targeted scan found **8 HIGH** vulnerabilities in signed plugins,
down from **9 HIGH** findings in the active Grafana recipe: the single Thrift
finding in the server binary was absent from the candidate scan. The candidate
