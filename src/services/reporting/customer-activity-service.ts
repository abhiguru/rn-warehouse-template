...
export async function getAllCustomerActivity(
  ...
  try {
    ...
    const fromDate =
      params.fromDate ?? getDateDaysAgo(params.daysBack ?? DEFAULT_DAYS_BACK);

    console.log(
      '[CustomerActivity] Calling get_all_customer_activity_summary with:',
      {
    ...
    console.timeEnd(
      '⏱️ [CustomerActivity] getAllCustomerActivity RPC duration'
    );
    ...
    console.log(
      `📊 [CustomerActivity] Response size: ${(dataSize / 1024).toFixed(2)} KB`
    );
    ...
    console.log(
      '[CustomerActivity] Raw keys:',
      data ? Object.keys(data) : 'null'
    );
    console.log(
      '[CustomerActivity] First 500 chars:',
    ...
    // Map customer rows
    const customers: CustomerActivityRow[] = (
...
export async function getCustomerActivityDetail(
  ...
  try {
    ...
    console.log(
      '[CustomerActivityDetail] Calling get_customer_activity_detail with:',
      {
    ...
    const dataSize = JSON.stringify(data).length;
    console.log(
      `📊 [CustomerActivityDetail] Response size: ${(dataSize / 1024).toFixed(2)} KB`
    );
    ...
    console.log(
      '[CustomerActivityDetail] Final keys:',
      responseData ? Object.keys(responseData) : 'null'
    );
    ...
    const summary: CustomerActivityDetailKPIs = {
      ...
      total_dispatch_quantity:
        responseData?.summary?.total_dispatch_quantity ?? 0,
      total_invoice_amount: responseData?.summary?.total_invoice_amount ?? 0,
      ...
      average_stock_age_days:
        responseData?.summary?.average_stock_age_days ?? 0,
    };

    // Map monthly trends
    const monthlyTrends: MonthlyTrendPoint[] = (
      responseData?.monthly_trends ?? []
    ).map((trend: any) => ({
      month: trend.month,
    ...
    // Map recent GRNs
    const recentGrns: RecentGRNPreview[] = (
      responseData?.recent_grns ?? []
    ).map((grn: any) => ({
      grn_id: grn.grn_id,
      gr_no: grn.gr_no,
    ...
    const recentDispatches: RecentDispatchPreview[] = (
      responseData?.recent_dispatches ?? []
    ).map((dispatch: any) => ({
    ...
    // Map top stock items
    const topStockItems: StockItemPreview[] = (
      responseData?.top_stock_items ?? []
    ).map((item: any) => ({
    ...
    // Map recent invoices
    const recentInvoices: RecentInvoicePreview[] = (
